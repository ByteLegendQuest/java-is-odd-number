...
`git clone https://github.com/ByteLegendQuest/java-is-odd-number`

A project directory named `java-is-odd-number` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-is-odd-number` directory which is created just now.
